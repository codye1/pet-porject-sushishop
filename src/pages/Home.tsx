...
import Banner from '../UI/main-banner/Banner';
import { useAppDispatch } from '../hooks';
import korzina, {deletetovar} from '../reducer/korzina';
...
const Home = () => {
    ...
    let deispatch= useAppDispatch()

    let {data,error,isLoading} = useGetSetsQuery()
    data &&
    setTimeout(()=>console.log(store.getState().korzina.tovar),5000)
    ...
    return (
        <div className='App'>
            ...
            <Swiper
                ...
                className="mySwiper"
                    >
                {slide.map((s)=><SwiperSlide key={s.key}><img src={s.img} alt="" /></SwiperSlide>)}
            </Swiper>
            <div className='list-up'>
                Sushi Master в Києві
            </div>
            </div>
                {data? <div className='containernav' style={{marginTop:'200px'}} >
                    <TovarList limit={[0,8]} tovars={data}/>
                </div> : <h1>Загрузка</h1> }
            <Banner/>
                {data? <div className='containernav' style={{marginTop:'150px'}}>
                <TovarList limit={[8,data.length]} tovars={data}/>
            </div> : <h1>Загрузка</h1> }
